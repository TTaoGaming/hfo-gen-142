...
for name in ('strife.json','splendor.json'):
    ...
    require(event['data']['status']=='OBSERVED' and event['data']['consumer_refs']==[])
    require(event['source']=='urn:example:storage-kata' and 'SYNTHETIC EXAMPLE ONLY' in event['data']['claim_ceiling'])
    ...
    for evidence in event['data']['evidence']:
        ...
        require(evidence['visibility']=='PUBLIC' and evidence['ref'].startswith('https://github.com/TTaoGaming/hfo-gen-142/blob/'))
...
print('PASS: 2 schema-valid SYNTHETIC examples; pairing, privacy fields, hash width, no invented trace or ConsumerAck')
