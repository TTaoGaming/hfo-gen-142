...
- **H3 ROACH_BURROW** — durable continuation/fan-in semantics and Cloudflare/GitHub ownership boundary; no live claim without assay.
- **H4 REDUCER_CONSUMERACK** — terminal fan-in, canonical Strife/Splendor/genes/world state and knowledge fitness through actual reuse.
- **H5 A2A_AGENT_CARD** — current official Agent Card conformance, live-fact materialization and candidate-card evidence matrix.
- **H6 AGENT_SKILLS** — smallest progressive-disclosure Skill taxonomy and activation/ablation rules; avoid one skill per metaphor.
- **H7 TOOLS_AUTHORITY** — MCP/provider-native/CLI tools, capability admission, credentials/scopes/effect ceilings and reference-monitor boundary.
- **H8 CLOUDFLARE_HIVE_RUNTIME** — exact native owner map for Agent/DO/Workflow/Fiber/Queue/RPC/AI Gateway; propose smallest R0 vertical slice, no custom duplicate runtime.
