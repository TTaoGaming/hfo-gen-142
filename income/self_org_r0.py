...
def main():
    ...
    for lane, spec in config["lanes"].items():
        ...
        if terminal_count < need or occupancy[lane] >= cap:
            continue
        score = int(spec["priority"]) - 18 * occupancy[lane]
        eligible.append({"lane":lane, "score":score, "occupancy":occupancy[lane], **spec})
    if not eligible:
        print(json.dumps({"decision":"HOLD","reason":"NO_ELIGIBLE_LANE","occupancy":occupancy,"terminals":terminal_count}, indent=2)); return 2
    ...
    out = {
      "schema":"hfo.gen142.income-claim.v1", "carrier_episode_uuid":episode,
      "utc":now, "control_issue":ISSUE, "selected_lane":candidate["lane"],
      "phenotype":candidate["phenotype"], "mission":candidate["mission"],
      "symbolic_score":candidate["score"], "occupancy_before":candidate["occupancy"],
      "terminal_count_observed":terminal_count, "neural_advisory":neural,
      "effect_ceiling":config["effect_ceiling"], "tao_relay_required":False,
      "next_action":"POST_CLAIM_THEN_EXECUTE_BOUNDED_LANE"
    }
